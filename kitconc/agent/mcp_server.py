...
def _register_tools(server, actions: KitconcActions) -> None:
    for tool in actions.mcp_tool_catalog():
        ...
        def make_handler(method_name: str):
            def _handler(**kwargs):
                try:
                    result = getattr(actions, method_name)(**kwargs)
            ...
            _handler.__doc__ = f"{description}\n\nInput: {json.dumps(tool['inputSchema'])}"
            return _handler

        server.tool(name=name, description=description)(make_handler(name))


def build_server(workspace: str | None = None, workspace_file: str | None = None):
    if FastMCP is None:
        raise RuntimeError(
...
def main() -> None:
    ...
    server = build_server(args.workspace, args.workspace_file)
