...
class Clusters(object):
    ...
    def save_xls(self,filename):
        ...
        worksheet.write('B1', 'CLUSTER',headers_format)
        worksheet.write('C1', 'FREQUENCY',headers_format)
        worksheet.write('D1', 'RANGE',headers_format)
        ...
        for row in self.df.itertuples(index=False):
            ...
            worksheet.write(i,1, str(row[1]),col_B_format) # N-GRAM 
            worksheet.write(i,2, int(row[2]),col_C_format) # FREQ
            worksheet.write(i,3, float(row[3]),col_D_format) # RANGE
