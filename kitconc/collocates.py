...
class Collocates(object):
    ...
    def save_xls(self,filename):
        ...
        worksheet.write('D1', 'LEFT',headers_format)
        worksheet.write('E1', 'RIGHT',headers_format)
        worksheet.write('F1', 'ASSOCIATION',headers_format)
        ...
        for row in self.df.itertuples(index=False):
            ...
            worksheet.write(i,3, float(row[3]),col_D_format) # LEFT
            worksheet.write(i,4, float(row[4]),col_E_format) # RIGHT
            worksheet.write(i,5, float(row[5]),col_F_format) # FORCE
        # close
        workbook.close()
