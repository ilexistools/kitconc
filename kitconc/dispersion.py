...
class Dispersion(object):
    ...
    def save_xls(self,filename):
        ...
        worksheet.write('B1', 'FILENAME',headers_format)
        worksheet.write('C1', 'TOTAL',headers_format)
        ...
        worksheet.write('L1', 'SECTION',headers_format)
        worksheet.write('M1', 'HITS',headers_format)
        worksheet.write('N1', '%',headers_format)
        
        worksheet.write('L2', 'S1',headers_format)
        worksheet.write('L3', 'S2',headers_format)
        worksheet.write('L4', 'S3',headers_format)
        worksheet.write('L5', 'S4',headers_format)
        worksheet.write('L6', 'S5',headers_format)
        
        worksheet.write('M2', int(self.total_s1) ,col_E_format)
        worksheet.write('M3', int(self.total_s2),col_E_format)
        worksheet.write('M4', int(self.total_s3),col_E_format)
        worksheet.write('M5', int(self.total_s4),col_E_format)
        worksheet.write('M6', int(self.total_s5),col_E_format)
        
        total = float(self.total_s1 + self.total_s2 + self.total_s3 + self.total_s4 + self.total_s5)
        
        worksheet.write('N2', round((self.total_s1 / total) * 100,2),col_E_format)
        worksheet.write('N3', round((self.total_s2 / total) * 100,2),col_E_format)
        worksheet.write('N4', round((self.total_s3 / total) * 100,2),col_E_format)
        worksheet.write('N5', round((self.total_s4 / total) * 100,2),col_E_format)
        worksheet.write('N6', round((self.total_s5 / total) * 100,2),col_E_format) 
        ...
        for row in self.df.itertuples(index=False):
            ...
            worksheet.write(i,1, str(row[1]),col_B_format) # FILENAME 
            worksheet.write(i,2, int(row[2]),col_C_format) # TOTAL
            if row[3] == 0:
