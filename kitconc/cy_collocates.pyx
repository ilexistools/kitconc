...
cdef tuple get_wordlist(str workspace,str corpus_name,object language,bint lowercase):
...
cdef tuple parse_search(str workspace,str corpus_name,str node,object pos,object case_sensitive,object regexp,object collpos):
...
cdef tuple search_node(str npy_path,object encoded_node,object encoded_pos,object encoded_collpos,int left_span,int right_span):
...
cdef np.ndarray collpos_filter(np.ndarray hor_arr,object encoded_collpos):
...
cdef tuple get_context(np.ndarray arr,np.ndarray indexes,int size,object encoded_collpos,int left_span,int right_span):
...
cdef float mutual_information(int ab,int a,int b,int N, int h):
...
cdef float tscore(int ab,int a,int b,int N,int h):
    """Calculates t-score."""
    cdef:
        float O 
        float E 
...
cdef list calc_association(int node_count,dict left_counts,dict right_counts,dict dict_words,dict wordlist,int tokens,object lowercase,int measure):
    ...
    if measure == 1:
        ...
        for w,v in d.items():
            if punct.match(w) == None:
                ...
                s.append("%s\t%s\t%s\t%s\t%s" % (w,v[0]+v[1],node_count,b,tokens) )
                collocates.append((i,w,v[0]+v[1],v[0],v[1],m)) 
    else:
        i = 0 
        for w,v in d.items():
            if punct.match(w) == None:
...
def make_collocates(str workspace,str corpus_name,str node,str pos,object case_sensitive,object regexp, object collpos,bint lowercase,int left_span,int right_span,int measure):
    cdef:
        object encoded_node
        object encoded_pos 
        object encoded_collpos 
